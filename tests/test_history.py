...
@pytest.fixture
def test_history_facade_fixture():
    """
...
def test_save_and_load(test_history_facade_fixture):
    """
    Test saving and loading calculations using HistoryFacade.
    """
    test_history_facade_fixture.save_calculation("add", [5, 3], 8)

    # Check if the saved calculation exists
    data = test_history_facade_fixture.load_history()
    assert len(data) == 1
    assert data.iloc[0]["Operation"] == "add"
    assert data.iloc[0]["Result"] == 8


def test_simplified_methods(test_history_facade_fixture):
    """
    Test simplified methods of the HistoryFacade.
    """
    test_history_facade_fixture.save_calculation("multiply", [2, 4, 2], 16)
    data = test_history_facade_fixture.load_history()
    assert len(data) == 1
    ...
    # Clear the history
    test_history_facade_fixture.clear_history()
    assert os.path.exists(test_history_facade_fixture.history_file)
